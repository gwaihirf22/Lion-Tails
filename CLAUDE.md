...
- **EVERY PICTURE ASKS FIRST** (`client/src/components/reader/PictureDialog.tsx`).
  ...
  - **It says what a picture costs**, from the published price list:
    `GET /api/pricing` sums the image row and the scene-writing row
    (`pictureListPrice`, `pictureItemKey`) because a picture is two paid calls,
    and which rows they are depends on the account's models. **Null until
    prices are published on `/admin/costs`**, and then the sentence reads a
    real number with no code change; with no price it says what is true in
