 ...
 *
 * IT OWNS ITS QUERIES, so every call site gets the same sentence with no prop
 * drilling; the keys are ones the reader has already fetched, so TanStack
 * serves them from the cache.
 *
 * TWO KINDS OF PRICE, AND THE ACCOUNT DECIDES WHICH IS TRUE. A picture costs
 * CREDITS (`PICTURE_CREDITS`, by tier), and that is what a family spends -- so
 * that is what the box says, with the balance beside it. An admin or an
 * account on its own key is charged no credits at all, and for them the honest
 * number is the money one, from the published price list (`/api/pricing`).
 */
export default function PictureDialog({
  ...
  // What this account is charged, and what it has left. Both keys are already
  // in the reader's cache -- the button's price comes from the same answer.
  const { data: modelInfo } = useQuery<{ pictures?: PictureSettings }>({
    queryKey: ["/api/settings/models"],
    enabled: open,
  });
  const { data: usage } = useQuery<StoryUsage>({ queryKey: ["/api/story/usage"], enabled: open });
  const pictures = modelInfo?.pictures;
  const credits = pictures && !pictures.free ? pictures.credits : 0;
  const tierLabel = pictures?.tiers.find((t) => t.tier === pictures.tier)?.label;

  /**
   * The price, in the unit this account actually pays in.
   *
   * Credits when it is charged them, and then the balance goes beside it so
   * nobody presses a button they cannot afford. Money when it is charged none
   * -- an admin or an own key -- and then only if a price has been published;
   * a number is never invented for the sake of having one.
   */
  const priceSentence =
    credits > 0
      ? `This picture costs ${creditsLabel(credits)}${tierLabel ? ` (${tierLabel.toLowerCase()})` : ""}` +
        `${usage ? `, and you have ${creditsLabel(usage.remaining)}` : ""}. It takes a few minutes.`
      : cents === null
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <p className="m-0 text-xs text-muted-foreground" data-guide="picture-cost">
          {priceSentence} This story has {galleryCount} of {MAX_STORY_IMAGES} pictures.
        </p>
